  ...
  <div class="terminal-output" ref="terminalOutput">
    <div class="show-previous-commands" id="prev-commands">
      <div v-for="(command, i) in storeCommand" :key="i">
        ...
        <div v-else>
          <p>
            <span class="invalid-command"
              >Invalid command : '{{ command }}'</span
            >. Try <span>'help'</span> to get started
          </p>
        </div>
      </div>
    </div>
    ...
    <div class="input-prompt m-30">
      ...
      <form @submit.prevent>
        ...
        <input
          ...
          for="input-prompt"
          aria-labelledby="input-prompt"
          id="command-input"
          :class="{
            'incorrect-command': !isCommandCorrect,
            'correct-command': isCommandCorrect,
          }"
          ref="inputField"
          type="text"
  ...
  methods: {
    ...
    displayCommandOutput() {
      if (this.inputValue === "clear") {
        ...
        window.open("https://github.com/echomaverick/portfolio", "_blank");
      } else {
        const command = this.inputValue.toLowerCase().trim();
        this.addToCommandHistory(command);
      }
